...
export default function DisputeResolution() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // Extract vehicle_id from invoice data for AriaChat
      if (invRes.data?.vehicle_id) {
        setVehicleId(invRes.data.vehicle_id);
      }
  ...
  const isResolutionComplete = !canResolveDispute && invoice.dispute_status && invoice.dispute_status !== null;
  ...
  return (
    <div className="max-w-7xl mx-auto" style={{maxWidth:900,margin:"0 auto",padding:"32px 40px"}}>
          ...
          {canResolveDispute ? (
            ...
            <div className="bg-white rounded-2xl border border-green-200 shadow-sm overflow-hidden">
              <div className="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
                <h2 className="text-white font-bold text-lg">✅ Dispute Resolved</h2>
                <p className="text-green-200 text-sm mt-0.5">
                  Your dispute has been recorded and saved to the immutable audit log.
                </p>
              </div>
              <div className="p-6 space-y-4">
                <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                  <p className="text-sm text-green-800">
                    <strong>Resolution Status:</strong> {invoice.dispute_status.replace(/_/g, ' ').toUpperCase()}
                  </p>
                  <p className="text-sm text-green-800 mt-2">
                    <strong>Resolved At:</strong> {new Date(invoice.dispute_resolved_at).toLocaleString()}
                  </p>
                </div>
                <Link 
                  to="/" 
                  className="inline-block w-full text-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-colors"
                >
                  ← Back to Dashboard
                </Link>
              </div>
      ...
      {/* ARIA chat widget */}
      <AriaChat vehicleId={vehicleId || null} vehicleName="" />
    </div>
  );
}
