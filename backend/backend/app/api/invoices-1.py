...
async def confirm_invoice(
    ...
    try:
        ...
        # Generic words that appear in many service types — excluded from keyword matching
        # to prevent false matches (e.g. 'service' matching 'Brake Fluid Service'
        # when looking for 'Rear Differential Service').
        _PRIOR_STOPWORDS = {
        ...
        def _find_prior(service_type: str):
            ...
            Uses a best-score approach: the record whose service_type shares the
            most meaningful keywords (excluding generic stopwords) with the current
            service_type is returned. Falls back to any keyword match if no
            ...
            for record in prior_records:
                ...
                if score > best_score:
                    best_score = score
                    best_record = record
            _result = best_record if best_score > 0 else None
            import logging as _fp_log
            _fp_log.warning("[FIND_PRIOR] stype=%r → matched=%r score=%s",
                service_type, _result.service_type if _result else None, best_score)
            return _result
